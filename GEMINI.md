...
- **Tryb Direct (Szybkie dowożenie):** Gemini generuje gotowy, kompletny i produkcyjny kod na żądanie, aby maksymalnie przyspieszyć tempo prac. Brak pytań naprowadzających – liczy się czas i realizacja checklisty MVP.
- **Priorytet Dokumentacji:** Wszelkie rozwiązania techniczne Gemini opiera na oficjalnej dokumentacji (np. PEP 8 dla Pythona) i standardach, a nie na popularnych hackach ze Stack Overflow. Jeśli rozwiązanie jest hackiem – Gemini musi to wyraźnie zaznaczyć.
- **Jedno zadanie naraz:** Realizujemy dokładnie jedno zadanie z MVP-checklisty w danym momencie. Po każdym kroku przetestuj endpoint w `/docs` lub w przeglądarce.
- **Zmiana logiki punktacji:** Najpierw piszemy/aktualizujemy test (`pytest`), a dopiero potem zmieniamy implementację w kodzie.
- **Równoległy rozwój:** Przed kodowaniem backendu i frontendu ustalamy sztywny kontrakt API (co endpoint przyjmuje i co dokładnie zwraca).
- **Komentarze i styl:** Komentarze w kodzie piszemy po angielsku, krótkie i proste (styl junior/mid). Opisują CO i PO CO, a nie JAK.
- **Żadnych motywacyjnych banałów:** Komunikacja ma być krótka, merytoryczna i szczera technicznie. Jeśli rozwiązanie niesie ryzyko architektoniczne lub bezpieczeństwa – Gemini mówi o tym wprost.

### Pętla pracy Gemini (Obowiązkowa)

Przy każdym zadaniu Gemini trzyma się tej pętli:

1. **Plan:** Zanim cokolwiek zmieni, pokazuje listę kroków/modyfikacji, które planuje zrobić i czeka na akceptację (chyba że zadanie jest trywialne).
2. **Implementacja:** Pisanie kodu małymi krokami, dbając o czysty SQL, brak ORM i separację warstw.
3. **Test:** Odpalenie testów, sprawdzenie endpointu w `/docs` lub weryfikacja poprawności SQL na bazie. Zadanie nie jest skończone bez tego kroku.
4. **Sanity check:** Weryfikacja, czy zmiana jest w 100% zgodna z architekturą i regułami zapisanymi w `GEMINI.md`.
5. **Raport zmian + propozycja commita:** Pokazuje statystyki zmian (ile linii dodano/usunięto per plik), krótkie jednozdaniowe podsumowanie zmian dla każdego pliku oraz gotową propozycję treści commita.

### Git operacje — Gemini NIGDY samodzielnie

Gemini **nie wykonuje żadnych operacji git**, kropka. Konkretnie zakazane:
- `git commit` (w tym `--amend`)
- `git checkout`, `git branch`, `git checkout -b`
- `git merge`, `git rebase`, `git push`, `git pull`, `git fetch`
- `git stash`, `git reset`, `git restore`
- Jakiekolwiek modyfikacje przez CLI `gh` (`gh pr create`, `gh pr merge` itp.)

**Wszystkie operacje git wykonują bartek lub Daniel ręcznie.** Gemini może jedynie:
- **Czytać** stan repozytorium do raportów: `git status`, `git log`, `git diff`, `git branch --show-current`, `gh pr view`.
- **Proponować i ostrzegać:** Pokazać treść commita, przypomnieć o zmianie brancha lub ostrzec, jeśli praca odbywa się bezpośrednio na `main`.

Jeśli Gemini widzi potrzebę wykonania operacji na Git – mówi o tym wprost w oknie czatu i czeka na ruch użytkownika. Przed każdą propozycją commita Gemini sprawdza aktualny branch (`git branch --show-current`) – jeśli to `main`, prosi o przełączenie i czeka.
---
